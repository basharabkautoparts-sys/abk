	...
	<div class="mt-6 grid gap-10 lg:grid-cols-2">
		...
		<div>
			<div class="flex items-center gap-2">
				...
				<span
					class="inline-flex items-center gap-1.5 rounded-full bg-abk-rose px-3 py-1 text-xs font-bold text-abk-red ring-1 ring-abk-red/15"
				>
			...
			<div class="mt-5 flex flex-wrap items-center gap-3">
				<span
					class="inline-flex items-center gap-2.5 rounded-xl border border-slate-200 bg-white px-4 py-2.5 shadow-[var(--shadow-card)]"
				>
					<Icon name="car" size={20} class="text-abk-red" />
					<span class="text-sm text-slate-500">Fits</span>
					<span class="text-lg font-black leading-none text-abk-blue">{part.brand.name}</span>
				</span>
				{#if part.in_stock}
					<span
			...
			<p class="mt-2 text-center text-xs text-slate-400 sm:text-left">
				Message us to confirm availability and get an export quote to your country.
			</p>
