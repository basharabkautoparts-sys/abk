<script lang="ts">
	...
	const cards = $derived([
		...
		{ label: 'Featured', value: data.stats.featured, icon: 'star', accent: 'text-abk-red bg-abk-rose' },
		...
		{#each data.recent as part (part.id)}
			<li>
				<a href={`/admin/parts/${part.id}/edit`} class="flex items-center gap-3 py-3 hover:bg-slate-50">
					...
					<span class="flex shrink-0 items-center gap-1 text-xs font-bold text-abk-red">
