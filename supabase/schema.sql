...
-- Existing installs only: the catalogue no longer stores or shows prices.
-- The app simply ignores these columns, so dropping them is optional — run these
-- two lines by hand once you are happy to lose the stored price data.
--   alter table public.parts drop column if exists price;
--   alter table public.parts drop column if exists currency;
